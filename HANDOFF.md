...
- **Self-service:** pass fields are self-editable directly (NO approval) on `/profile` via `ProfileExtras` (+ "My Documents" manager). Admin edits anyone's pass fields in the `/admin/users` Edit dialog (staff roles only); per-surveyor doc management at `/admin/users/[id]/documents`. Office (with `personal_docs.view`) gets read-only `/office/documents` (copy details + download). New office permission toggles appear automatically (catalog-driven).
...
## Pending migrations to run (in order): **029** (vessel documents), **030** (realtime_jobs), **031** (profile_change_requests), **035** (personal documents). Plus set `CRON_SECRET` in Vercel.
