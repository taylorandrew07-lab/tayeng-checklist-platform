    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1">{loading ? '…' : `${jobs.length} jobs`}</p>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                ...
                <tr><td colSpan={7} className="px-4 py-10 text-center text-gray-400">No jobs yet. <Link href="/admin/jobs/new" className="text-brand-600 hover:underline">Create one →</Link></td></tr>
              ) : jobs.map((job) => (
                <tr key={job.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-gray-500">{formatDate(job.scheduled_date)}</td>
                  <td className="px-4 py-3">
